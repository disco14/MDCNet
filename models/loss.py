...
def stereo_psmnet_loss(inputs, target, mask, **kwargs):
    ...
    for (stage_inputs, stage_key) in [(inputs[k], k) for k in inputs.keys() if "stage" in k]:
        ...
        # loss = 0 * F.smooth_l1_loss(disp0[mask], target[mask], reduction='mean') + \
        #        0 * F.smooth_l1_loss(disp1[mask], target[mask], reduction='mean') + \
        #        0 * F.smooth_l1_loss(disp2[mask], target[mask], reduction='mean') + \
        #        1.0 * F.smooth_l1_loss(disp3[mask], target[mask], reduction='mean')
